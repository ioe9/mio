...
class Mage_Widget_Adminhtml_Widget_InstanceController extends Mage_Adminhtml_Controller_Action
{
    ...
    /**
     * Posts chooser Action (Ajax request)
     *
     */
    public function postsAction()
    {
        $selected = $this->getRequest()->getParam('selected', '');
        $postTypeId = $this->getRequest()->getParam('post_type_id', '');
        $chooser = $this->getLayout()
            ->createBlock('adminhtml/catalog_post_widget_chooser')
            ->setName(Mage::helper('core')->uniqHash('posts_grid_'))
            ->setUseMassaction(true)
            ->setPostTypeId($postTypeId)
            ->setSelectedPosts(explode(',', $selected));
        ...
        $serializer->initSerializerBlock($chooser, 'getSelectedPosts', 'selected_posts', 'selected_posts');
