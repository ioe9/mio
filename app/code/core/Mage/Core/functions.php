...
if (!function_exists("pagination")) {
	function pagination($total, $pageSize = 10,$url) {
		$page_count  = ceil($total/$pageSize);  //计算得出总页数
	    $init=1;
	    $page_len=10;
	    $max_p=$page_count;
	    $pages=$page_count;
	    ...
	    if($pages != 0){
	        if($page!=1){
	            ...
	            $navs .= "<li><span class='disabled'>上一页</span></li>";
	        }
	        if($pages>$page_len)
	        {
	            //如果当前页小于等于左偏移
	        ...
	        {
	            if($i==$page){$navs.="<li><span class='current'>".$i.'</span></li>';} 
	            else {$navs.="<li><a href=\"".$url."page=".$i."\">".$i."</a></li>";}
	        }
	        $navs.='<li class="pager-ellipsis disabled"><span>…</span></li>';
	        if($page!=$pages)
	        {
	            $navs.="<li><a href=\"".$url."page=".($page+1)."\">下一页</a></li>";//下一页
	        } else {
	            $navs .= "<span class='disabled'>下一页</span></li>";
	        }
	        echo "$navs";
	    }
	}
