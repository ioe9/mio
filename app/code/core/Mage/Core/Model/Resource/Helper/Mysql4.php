            ...
            foreach ($selectHavings as $having) {
                ...
                if (strpos($having, $correlationName) !== false) {
                    if (is_string($column)) {
                        ...
                        throw new Varien_Db_Exception(sprintf("Can't prepare expression without column alias: '%s'", $correlationName));
                    }
                }
            }
        }
        ...
        foreach ($columns as $columnEntry) {
            ...
            if ($column instanceof Magento_Db_Expr) {
                if ($alias !== null) {
                    ...
                    throw new Varien_Db_Exception("Can't prepare expression without alias");
                }
            } else {
                if ($column == Varien_Db_Select::SQL_WILDCARD) {
                    if ($tables[$correlationName]['tableName'] instanceof Magento_Db_Expr) {
                        throw new Varien_Db_Exception("Can't prepare expression when tableName is instance of Magento_Db_Expr");
                    }
