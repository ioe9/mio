 ...
 *      )
 *  ));
 *  $filter->addFilter('name2', new Varien_Filter_Alnum());
 *  $filter->addFilter('name1',
 *      array(
 *          'zend' => 'StringToUpper',
 *          'args' => array('encoding' => 'utf-8')));
 *  $filter->addFilter('name1', array('zend' => 'StripTags'), Varien_Filter::CHAIN_PREPEND);
 ...
 */
class Mage_Core_Model_Input_Filter implements Varien_Filter_Interface
{
    /**
     ...
     *
     * @param string $name
     * @param array|Varien_Filter_Interface $filter
     * @param string $placement
     * @return Mage_Core_Model_Input_Filter
     */
    public function addFilter($name, $filter, $placement = Varien_Filter::CHAIN_APPEND)
    {
        if ($placement == Varien_Filter::CHAIN_PREPEND) {
     ...
     * @return Mage_Core_Model_Input_Filter
     */
    public function appendFilter(Varien_Filter_Interface $filter)
    {
        return $this->addFilter($filter, Varien_Filter::CHAIN_APPEND);
    }
    ...
    {
        return $this->addFilter($filter, Varien_Filter::CHAIN_PREPEND);
    }
        ...
        foreach ($data as $key => $value) {
            ...
            if (!$isFilterListSimple && is_array($value) && isset($filters[$key]['children_filters'])) {
                ...
                foreach ($itemFilters as $filterData) {
                    if ($zendFilter = $this->_getVarienFilter($filterData)) {
    ...
    /**
     * Try to create Varien filter based on $filterData. Return false on failure
     *
     * @param $filterData
     * @return bool|Varien_Filter_Interface
     */
    protected function _getVarienFilter($filterData)
    {
        $zendFilter = false;
        if (is_object($filterData) && $filterData instanceof Varien_Filter_Interface) {
            /** @var $zendFilter Varien_Filter_Interface */
            $zendFilter = $filterData;
        } elseif (isset($filterData['model'])) {
            $zendFilter = $this->_createCustomVarienFilter($filterData);
        } elseif (isset($filterData['zend'])) {
            $zendFilter = $this->_createNativeVarienFilter($filterData);
        }
        return $zendFilter;
    }
     ...
     *
     * @param $filterData
     * @return Varien_Filter_Interface
     * @throws Exception
     */
    protected function _createCustomVarienFilter($filterData)
    {
        ...
        if (!($filter instanceof Varien_Filter_Interface)) {
            throw new Exception('Filter is not instance of Varien_Filter_Interface');
        }
        return $filter;
    }

    /**
     * Get native Varien_Filter
     *
     * @param $filterData
     * @return Varien_Filter_Interface
     * @throws Exception
     */
    protected function _createNativeVarienFilter($filterData)
    {
        $filter = $filterData['zend'];
        if (is_string($filter)) {
            $class = new ReflectionClass('Varien_Filter_' . $filter);
            if ($class->implementsInterface('Varien_Filter_Interface')) {
