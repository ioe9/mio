        ...
        if ($catalogAction = $this->getRequest()->getPost('catalog_action')) {
            ...
            switch ($catalogAction) {
                ...
                case 'clear_images_cache':
                    try {
                        Mage::getModel('catalog/post_image')->clearCache();
                        $this->_getSession()->addSuccess(
                ...
                case 'rebuild_flat_catalog_post':
                    try {
                        Mage::getResourceModel('catalog/post_flat_indexer')->rebuild();
                        $this->_getSession()->addSuccess(Mage::helper('adminhtml')->__('The Flat Catalog Post was rebuilt'));
                    }
                    catch (Mage_Core_Exception $e) {
                        $this->_getSession()->addError($e->getMessage());
                    }
                    catch (Exception $e) {
                        $this->_getSession()->addException($e, Mage::helper('adminhtml')->__('An error occurred while rebuilding the flat post catalog.'));
                    }
                    break;
