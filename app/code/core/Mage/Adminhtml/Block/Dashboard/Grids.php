...
class Mage_Adminhtml_Block_Dashboard_Grids extends Mage_Adminhtml_Block_Widget_Tabs
{
    ...
    protected function _prepareLayout()
    {
        ...
        $this->addTab('ordered_posts', array(
            'label'     => $this->__('Bestsellers'),
            'content'   => $this->getLayout()->createBlock('adminhtml/dashboard_tab_posts_ordered')->toHtml(),
            'active'    => true
        ));

        // load other tabs with ajax
        $this->addTab('reviewed_posts', array(
            'label'     => $this->__('Most Viewed Posts'),
            'url'       => $this->getUrl('*/*/postsViewed', array('_current'=>true)),
            'class'     => 'ajax'
        ));
