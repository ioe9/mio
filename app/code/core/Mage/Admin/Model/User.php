...
class Mage_Admin_Model_User extends Mage_Core_Model_Abstract
{
    ...
    public function validate()
    {
        ...
        if (!Varien_Validate::is($this->getFirstname(), 'NotEmpty')) {
            $errors[] = Mage::helper('adminhtml')->__('First Name is required field.');
        }

        if (!Varien_Validate::is($this->getLastname(), 'NotEmpty')) {
        ...
        if ($this->hasPassword()) {
            if (Mage::helper('core/string')->strlen($this->getPassword()) < self::MIN_PASSWORD_LENGTH) {
                $errors[] = Mage::helper('adminhtml')->__('Password must be at least of %d characters.', self::MIN_PASSWORD_LENGTH);
            }

            if (!preg_match('/[a-z]/iu', $this->getPassword())
                || !preg_match('/[0-9]/u', $this->getPassword())
            ) {
