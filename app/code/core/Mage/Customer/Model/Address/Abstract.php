...
class Mage_Customer_Model_Address_Abstract extends Mage_Core_Model_Abstract
{
    ...
    {
        return $this->getUsername()."<br/>".$this->getProvince().$this->getCity().$this->getArea().$this->getStreet()."<br/>";
    }
    ...
    protected function _basicCheck()
    {
        if (!Varien_Validate::is($this->getUsername(), 'NotEmpty')) {
            $this->addError("收件人不能为空");
        }
	
        if (!Varien_Validate::is($this->getStreet(), 'NotEmpty')) {
            $this->addError(Mage::helper('customer')->__('Please enter the street.'));
        }

        if (!Varien_Validate::is($this->getCity(), 'NotEmpty')) {
            $this->addError(Mage::helper('customer')->__('Please enter the city.'));
        }

        if (!Varien_Validate::is($this->getTelephone(), 'NotEmpty')) {
        ...
        if (!in_array($this->getCountryId(), $_havingOptionalZip)
            && !Varien_Validate::is($this->getPostcode(), 'NotEmpty')
        ) {
        ...
        if ($this->getCountryModel()->getRegionCollection()->getSize()
            && !Varien_Validate::is($this->getRegionId(), 'NotEmpty')
